...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@500;600;700&family=Orbitron:wght@700;900&family=Exo+2:wght@400;500;600;700;800&display=swap');
...
section[data-testid="stSidebar"] * { color: #ffffff !important; font-family: 'Exo 2', sans-serif !important; }
...
.eco-header h1 {
    ...
    letter-spacing: 8px;
    margin: 0;
    text-shadow: 0 0 30px rgba(0,201,167,0.7), 0 0 60px rgba(0,201,167,0.3);
}
.eco-header p {
...
.metric-value {
    ...
    font-weight: 700;
    color: #00c9a7;
    text-shadow: 0 0 15px rgba(0,201,167,0.5);
}
.metric-label {
    font-family: 'Exo 2', sans-serif !important;
    font-size: 0.82rem;
...
.alert-critical { background: rgba(220,53,69,0.2); border-left: 4px solid #ff4444; color: #ffcccc; }
.alert-warning  { background: rgba(255,193,7,0.15); border-left: 4px solid #ffc107; color: #fff0a0; }
.alert-success  { background: rgba(0,201,167,0.15); border-left: 4px solid #00c9a7; color: #b0f0e0; }

/* LABELS */
label { color: #ffffff !important; font-size: 1rem !important; font-weight: 600 !important; font-family: 'Exo 2', sans-serif !important; }
...
p { color: #e8eaed !important; font-size: 1rem; }
h1, h2, h3, h4 { color: #ffffff !important; font-weight: 700; font-family: 'Exo 2', sans-serif !important; }
.stMarkdown p { color: #e8eaed !important; }
</style>
...
with tab1:
    ...
    def colour_risk(val):
        ...
        return c.get(val,'')
    st.dataframe(
        disp.style.map(colour_risk, subset=['Risk Level']).format({'Water Used (L)':'{:.0f}'}),
...
with tab2:
    ...
    with cl:
        ...
        ax.set_ylabel('Water Used (L)', fontsize=12)
...
with tab3:
    ...
    with x1:
        ...
        <b style="color:#00c9a7;font-size:1.05rem;letter-spacing:2px;font-family:'Rajdhani',sans-serif;">HOW TO READ SHAP</b><br><br>
        <span style="color:#ffffff;font-size:1rem;line-height:2.4;font-family:'Exo 2',sans-serif;">
...
with tab4:
    ...
    for col,val,label,colour in [
        ...
        with col:
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-value" style="color:{colour};text-shadow:0 0 15px {colour}88;">{val}</div>
                <div class="metric-label">{label}</div>
            </div>""", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    if predict_btn:
        ...
        <div style="background:#162032;border:3px solid {rc};border-radius:16px;
                    padding:2rem;text-align:center;margin:1.5rem 0;box-shadow:0 0 25px {rc}44;">
            <div style="font-family:'Rajdhani',sans-serif;font-size:1rem;color:#ffffff;letter-spacing:4px;font-weight:700;margin-bottom:0.5rem;">PREDICTED WATER CONSUMPTION</div>
            <div style="font-family:'Orbitron',monospace;font-size:3.5rem;color:{rc};font-weight:900;text-shadow:0 0 20px {rc}88;">{pred:,.0f} L</div>
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.2rem;color:{rc};font-weight:800;letter-spacing:4px;margin-top:0.5rem;">{rl}</div>
        </div>""", unsafe_allow_html=True)
...
st.markdown("---")
st.markdown("""
<div style="text-align:center;color:#00c9a7;font-family:'Rajdhani',sans-serif;font-size:1rem;
